//! Markdown → HTML rendering (extracted from md-preview into its own crate).
//!
//! This is the **pure** rendering surface: it turns Markdown into HTML with zero
//! web/async/CRDT dependencies, so it is reusable on its own (tests, the
...
//! HTML document (styles, KaTeX, copy-button script).
...
pub fn render_mermaid_block(code: &str) -> String {
